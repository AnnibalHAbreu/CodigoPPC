...
Eliminar a necessidade de recompilação e download do projeto CODESYS sempre que um dispositivo Modbus (inversor, medidor, BESS) for alterado, adicionado ou substituído. A comunicação Modbus RTU passa a ser totalmente programática, usando a biblioteca `ModbusFB` do CODESYS, com os mapas de registros carregados de arquivos CSV externos no boot do controlador.

### 1.1 Abordagem Anterior (substituída)

Comunicação configurada via Device Tree do CODESYS (Modbus COM → Master → Slaves → Channels), usando `IoDrvModbus.ModbusChannel` com referência a `ModbusSlaveComPort`. Polling de 5ms. Endereços, function codes e mapeamento de variáveis definidos no IDE. Qualquer alteração exigia editar o projeto, recompilar e fazer download.
...
VAR_GLOBAL
    ...
    // COM1: índices 0..14 = inversores, COM2: índices 15..29 = inversores
    arInvActivePower    : ARRAY[0..29] OF REAL;     // kW por inversor
    arInvReactivePower  : ARRAY[0..29] OF REAL;     // kvar por inversor
    arInvVoltage        : ARRAY[0..29] OF REAL;     // V por inversor
    arInvStatus         : ARRAY[0..29] OF UINT;     // Status word por inversor
    
    // Medidor (COM1, device index 0 do canal)
    ...
    uiOnlineInvCount    : UINT;                     // Inversores respondendo
    bMeterOnline        : BOOL;                     // Medidor respondendo
    
    // ─── Diagnóstico (escritos por Task_ModbusComm) ──────────────
    // COM1: 0=medidor, 1..15=inversores; COM2: 0..14=inversores
    astDiagCOM1         : ARRAY[0..15] OF ST_CommDiagnostics;
    astDiagCOM2         : ARRAY[0..14] OF ST_CommDiagnostics;
    
    // ─── Dados de escrita (escritos por Task_Main) ───────────────
    // COM1: índices 0..14, COM2: índices 15..29
    arInvSetpointP      : ARRAY[0..29] OF REAL;     // kW setpoint por inversor
    arInvSetpointQ      : ARRAY[0..29] OF REAL;     // kvar setpoint por inversor
    auiInvControlWord   : ARRAY[0..29] OF UINT;     // Control word por inversor
    bWritePending       : BOOL;                     // TRUE = novos setpoints depositados
    
    // ─── Configuração (escrita pelo ConfigLoader no boot) ────────
    uiDeviceCountCOM1   : UINT;                     // Máx 16 (1 medidor + 15 inversores)
    uiDeviceCountCOM2   : UINT;                     // Máx 15 inversores
    astProfilesCOM1     : ARRAY[0..15] OF ST_DeviceProfile;
    astProfilesCOM2     : ARRAY[0..14] OF ST_DeviceProfile;
    bConfigLoaded       : BOOL;
END_VAR
```
...
|---|---|---|
| COM1 | Inversores + 1 Medidor | Até 15 inversores + 1 medidor = 16 devices |
| COM2 | Inversores | Até 15 inversores |
| **Total** | | **Até 31 devices** |

Isso define o dimensionamento dos arrays em `GVL_CommData` e `ST_DeviceProfile`:

```
COM1: devices[0]     = Medidor (Slave 100)
      devices[1..15] = Inversores (Slave 101..115)

COM2: devices[0..14] = Inversores (Slave 201..215)
```

**Implicação no tempo de polling:**

A 9600 baud, cada transação FC03 de leitura de ~20 registros consome ~25ms no barramento (request + response + inter-frame delay).

| Porta | Devices | Grupos leitura (est.) | Transações/ciclo | Tempo estimado |
|---|---|---|---|---|
| COM1 | 16 | ~32 leituras + escritas esporádicas | ~35 | **~900ms** |
| COM2 | 15 | ~30 leituras + escritas esporádicas | ~33 | **~850ms** |

Ambas dentro do poll interval de 1000ms. COM1 e COM2 operam em paralelo (barramentos RS-485 independentes, cada um com seu `ClientSerial`), portanto o tempo total do sistema não é a soma, mas o máximo dos dois: **~900ms**.
...
| COM2 | 15 inversores | ~33 | ~850ms |
| **Sistema** | **31 devices** | | **~900ms** (paralelo) |
...
A máquina de estados do MainProgram (START → READ → CONTROL → WRITE → IDLE → FAIL → STOP) **não muda**. O fluxo permanece idêntico. O que muda é a camada de transporte subjacente.
...
| `FB_WriteCom` | Orquestra escrita sequencial em 15 inversores × N canais por porta COM, usando `FB_ModBus` internamente | **Reescrito.** Nova versão usa `ClientRequestWrite*` ao invés de `FB_ModBus` + triggers. A lógica de sequenciamento (inv 1..15, canal 0..N) permanece, mas o mecanismo de transporte muda |
| `FB_CheckInversores` | Varredura de presença no startup: dispara 1 transação por inversor, conta respondentes | **Reescrito.** Mesma lógica, mas usando `ClientRequestReadHoldingRegisters` ao invés de `FB_ModBus` + triggers |
| `ModbusChannel` (instância `fbChannel`) | Usado no MainProgram para leitura via Device Tree channels | **Eliminado.** Substituído por `ClientRequestReadHoldingRegisters` / `ClientRequestReadInputRegisters` disparados diretamente |
...
As seguintes variáveis de `GVL_Main` que eram ligadas ao Device Tree **são eliminadas**:

```
// ELIMINAR — pertenciam ao mecanismo de triggers/Device Tree:
...
Substituídas pelos profiles em `GVL_CommData.astProfilesCOM1/COM2` e pelos dados em `GVL_CommData.arInv*`, `rMeter*`.
